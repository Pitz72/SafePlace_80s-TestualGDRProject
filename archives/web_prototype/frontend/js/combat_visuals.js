...
const CombatVisuals = {
     ...
     * @returns {Promise} Promise che si risolve quando l'animazione è completata
     */
    async showCombatWithNarrativeEffects(combatResult, enemy) {
        console.log('[COMBAT_VISUAL] Inizio animazione combattimento vs', enemy.name);
        ...
        // Mostra popup preparazione al combattimento
        if (typeof showEventPopup === 'function') {
            showEventPopup({
                title: "⚔️ Preparazione al Combattimento",
                description: combatContainer.outerHTML,
                isNarrative: true
            });
        }
        
        // ASPETTA che l'animazione si completi DAVVERO prima di tornare
        console.log('[COMBAT_VISUAL] Avvio animazione completa...');
        await this.animateAdvancedCombat(combatResult, enemy, combatContainer);
        
        // ASPETTA anche il risultato finale
        console.log('[COMBAT_VISUAL] Mostrando risultato finale...');
        await this.showFinalResult(combatResult, enemy);
        
        console.log('[COMBAT_VISUAL] ✅ Animazione VERAMENTE completata!');
    ...
    async animateAdvancedCombat(combatResult, enemy, container) {
        console.log('[COMBAT_VISUAL] Inizio animazione rounds...');
        ...
        console.log('[COMBAT_VISUAL] Animando', combatResult.rounds.length, 'round...');
        for (let i = 0; i < Math.min(combatResult.rounds.length, 12); i++) {
            const round = combatResult.rounds[i];
            console.log(`[COMBAT_VISUAL] Round ${i+1}/${combatResult.rounds.length}`);
        ...
        console.log('[COMBAT_VISUAL] Round completati, pausa di suspense...');
        
        // Pausa di suspense prima del risultato finale
        await this.delay(this.config.suspenseDuration);
        
        console.log('[COMBAT_VISUAL] Mostrando risultato in-popup...');
        
        // Mostra il risultato finale nel popup corrente
        const finalResultElement = container.querySelector('#combat-result-final');
        finalResultElement.style.display = 'block';
        
        if (combatResult.victory) {
            finalResultElement.innerHTML = `
                <div class="victory-message" style="color: ${this.config.colors.victory}">
                    <h4>🎉 VITTORIA!</h4>
                    <p>Hai sconfitto ${enemy.name}!</p>
                    ${combatResult.expGained ? `<p>+${combatResult.expGained} punti esperienza</p>` : ''}
                </div>
            `;
            this.triggerVictoryEffects(container);
        } else {
            finalResultElement.innerHTML = `
                <div class="defeat-message" style="color: ${this.config.colors.defeat}">
                    <h4>💀 SCONFITTA</h4>
                    <p>${enemy.name} ti ha sopraffatto!</p>
                    <p>-${combatResult.damageTaken} HP</p>
                </div>
            `;
            this.triggerDefeatEffects(container);
        }
        
        console.log('[COMBAT_VISUAL] Pausa finale prima di completare...');
        
        // Pause finale per mostrare risultato
        await this.delay(4000); // 4 secondi per vedere il risultato
        
        console.log('[COMBAT_VISUAL] ✅ Animazione rounds completata');
    },
    ...
    /**
     * Mostra risultato finale del combattimento
     */
    async showFinalResult(combatResult, enemy) {
        console.log('[COMBAT_VISUAL] Show final result chiamata');
        
        // Il risultato è già mostrato nel popup precedente, quindi semplicemente
        // chiudo il popup corrente e ne mostro uno pulito finale
        if (typeof closeEventPopup === 'function') {
            closeEventPopup();
        }
        
        // Piccola pausa per la transizione
        await this.delay(500);
        
        const title = combatResult.victory ? "🎉 VITTORIA!" : "💀 SCONFITTA";
        
        let description = `<div class="combat-final-result">`;
        description += `<h3>${title}</h3>`;
        
        if (combatResult.victory) {
            description += `<p style="color: ${this.config.colors.victory}">Hai sconfitto ${enemy.name}!</p>`;
            if (combatResult.expGained > 0) {
                description += `<p>+${combatResult.expGained} punti esperienza guadagnati!</p>`;
            }
        } else {
            description += `<p style="color: ${this.config.colors.defeat}">${enemy.name} ti ha sopraffatto!</p>`;
            description += `<p>Hai subito ${combatResult.damageTaken} danni totali.</p>`;
        }
        
        description += `</div>`;
        
        // Mostra risultato finale pulito
        if (typeof showEventPopup === 'function') {
            showEventPopup({
                title: title,
        ...
        console.log('[COMBAT_VISUAL] ✅ Final result mostrato');
    }
};
