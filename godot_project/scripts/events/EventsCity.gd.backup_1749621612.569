# 🏙️ EVENTS CITY - THE SAFE PLACE v1.3.0
# City/Urban specific events module
# Total target: ~300 eventi CITY

class_name EventsCity
extends RefCounted

static func get_events_database() -> Dictionary:
	return {
		"city_shadows": {
			"id": "city_shadows",
			"name": "Ombre tra i Grattacieli",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Scheletri di grattacieli graffiano il cielo plumbeo. Il vento fischia tra le finestre rotte come un lamento. Pericolo e tesori dimenticati si nascondono in ogni angolo.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Esplora un palazzo (Presagio)",
					"requirements": {"presagio": 13},
					"consequences": {
						"action": "skill_check",
						"stat": "presagio",
						"difficulty": 13,
						"success": {
							"text": "Il tuo istinto ti guida verso un edificio che sembra meno pericolante. All'interno, tra le macerie, trovi provviste, materiali e forse qualcosa per difenderti.",
							"rewards": {"items": {"canned_food": 1, "water_bottle": 1, "scrap_metal": 2, "bandages_clean": 1, "weapon_pipe": 1}}
						},
						"failure": {
							"text": "Il palazzo è un labirinto di pericoli e vicoli ciechi. Non trovi nulla di utile e rischi di perderti o peggio. Meglio ritirarsi.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Attraversa la strada",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Corri attraverso lo spazio aperto, sentendo gli occhi invisibili delle finestre vuote su di te. Raggiungi l'altro lato senza incidenti, per ora.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_easter_egg_pixeldebh": {
			"id": "city_easter_egg_pixeldebh",
			"name": "Strano Ritrovamento Metallico",
			"type": 5,  # SPECIAL
			"description": "Rovistando tra detriti metallici e cemento sbriciolato, noti uno strano oggetto lucido. Una placca argentata con un simbolo insolito e la scritta parzialmente leggibile 'PixelDebh'.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Raccogliere la placca",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Raccogli la placca misteriosa. Potrebbe essere un oggetto da collezione di tempi migliori.",
						"rewards": {"items": {"scrap_metal": 1, "lore_fragment": 1}}
					}
				},
				{
					"text": "Lasciarla lì",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Decidi di lasciare la placca dove l'hai trovata. Forse ha un significato per qualcun altro.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_unique_webradio": {
			"id": "city_unique_webradio",
			"name": "Studio Radio Silenzioso",
			"type": 5,  # SPECIAL
			"description": "Una stanza devastata: pareti annerite, macerie, monitor infranti su un tavolo circolare. Un'insegna sbrecciata sul muro recita 'R...me...adi'. Qui un tempo pulsava una WebRadio, voce libera ora ridotta al silenzio.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Cercare attrezzature (Tracce)",
					"requirements": {"tracce": 12},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 12,
						"success": {
							"text": "Tra i detriti trovi componenti elettronici ancora funzionanti e cavi utilizzabili.",
							"rewards": {"items": {"mechanical_parts": 2, "scrap_metal": 1, "lore_fragment": 1}}
						},
						"failure": {
							"text": "Tutto è troppo danneggiato per essere recuperato. Solo silenzio e polvere.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Osservare in silenzio",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Ti fermi a contemplare questo simbolo di libertà perduta. Un momento di riflessione in un mondo distrutto.",
						"rewards": {}
					}
				}
			]
		},

		"city_medical_supply": {
			"id": "city_medical_supply",
			"name": "Farmacia Saccheggiata",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Le insegne sbiadite di una farmacia. Dentro, scaffali rovesciati e blister vuoti ovunque. Forse è rimasto qualcosa di utile tra il disastro.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Cerca medicine (Adattamento)",
					"requirements": {"adattamento": 13},
					"consequences": {
						"action": "skill_check",
						"stat": "adattamento",
						"difficulty": 13,
						"success": {
							"text": "Con pazienza, rovistando tra scatole rotte e vetri infranti, la tua mano si chiude su alcune forniture mediche!",
							"rewards": {"items": {"antidote": 1, "vitamins": 1, "bandages_clean": 2}}
						},
						"failure": {
							"text": "Saccheggiata a fondo. Trovi solo confezioni vuote e odore di disinfettante stantio.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Cercare attrezzature mediche",
					"requirements": {"tracce": 12},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 12,
						"success": {
							"text": "Tra i detriti trovi alcuni strumenti medici ancora utilizzabili.",
							"rewards": {"items": {"first_aid_kit": 1, "mechanical_parts": 1}}
						},
						"failure": {
							"text": "Tutto è stato saccheggiato o distrutto. Solo vetri rotti e polvere.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Uscire dalla farmacia",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Decidi che il rischio di vetri rotti e possibili contaminazioni non vale la pena.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_teen_gang_territory": {
			"id": "city_teen_gang_territory",
			"name": "Territorio di Banda Giovanile",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Graffiti colorati segnano questo quartiere come territorio di una banda di ragazzi sopravvissuti. Simboli minacciosi ma anche disegni che rivelano la loro giovane età. Segnali contrastanti di pericolo e di possibilità di contatto.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Tentare un contatto (Influenza)",
					"requirements": {"influenza": 13},
					"consequences": {
						"action": "skill_check",
						"stat": "influenza",
						"difficulty": 13,
						"success": {
							"text": "Dopo un teso confronto iniziale, riesci a guadagnarti la fiducia del gruppo. Ti offrono una balestra semplice e qualche dardo.",
							"rewards": {"items": {"crossbow_simple": 1, "ammo_bolt": 3}}
						},
						"failure": {
							"text": "Un fischio acuto risuona tra gli edifici. Sagome si muovono come ombre. 'Questo non è territorio tuo,' grida una voce giovane. Indietreggi lentamente, sentendoti osservato.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Studiare i loro nascondigli (Tracce)",
					"requirements": {"tracce": 14},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 14,
						"success": {
							"text": "Evitando di farti notare, scopri uno dei loro depositi. Prendi cibo, acqua e qualche risorsa.",
							"rewards": {"items": {"canned_food": 2, "water_bottle": 1, "scrap_metal": 1, "ammo_generic": 5}}
						},
						"failure": {
							"text": "Un suono metallico scatta vicino al tuo piede. Una trappola rudimentale ma efficace. Un avvertimento. Allontanati rapidamente, sentendoti fortunato.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Evitare la zona",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Decidi che non vale la pena rischiare un confronto. Ti allontani discretamente dalla zona.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_devastated_library": {
			"id": "city_devastated_library",
			"name": "Biblioteca Devastata",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Entri in quella che un tempo era una biblioteca pubblica. Scaffali rovesciati, libri ammuffiti e pagine strappate coprono il pavimento. Forse qualche conoscenza è sopravvissuta.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Cercare libri utili (Adattamento)",
					"requirements": {"adattamento": 12},
					"consequences": {
						"action": "skill_check",
						"stat": "adattamento",
						"difficulty": 12,
						"success": {
							"text": "Tra i libri rovinati trovi manuali di sopravvivenza e mappe della zona ancora leggibili.",
							"rewards": {"items": {"lore_fragment": 2, "map_fragment": 1, "first_aid_kit": 1}}
						},
						"failure": {
							"text": "I libri sono troppo danneggiati dall'umidità e dal tempo. Solo carta marcita.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Usare come combustibile",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Raccogli libri secchi per accendere fuochi. La conoscenza brucia, ma il calore è vita.",
						"rewards": {"items": {"charcoal": 3}}
					}
				},
				{
					"text": "Cercare un rifugio",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Trovi un angolo tranquillo tra gli scaffali per riposare. Il silenzio della biblioteca è quasi pacifico.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_abandoned_subway": {
			"id": "city_abandoned_subway",
			"name": "La Stazione della Metropolitana Abbandonata",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Le scale mobili arrugginite conducono verso il basso, nell'oscurità di una stazione della metropolitana. L'aria è viziata e si sentono strani rumori provenire dalle gallerie.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Esplorare i tunnel (Presagio)",
					"requirements": {"presagio": 14},
					"consequences": {
						"action": "skill_check",
						"stat": "presagio",
						"difficulty": 14,
						"success": {
							"text": "Il tuo istinto ti guida attraverso i tunnel sicuri. Trovi un deposito di emergenza intatto.",
							"rewards": {"items": {"ration_pack": 2, "water_bottle": 2, "weapon_knife": 1, "first_aid_kit": 1}}
						},
						"failure": {
							"text": "I tunnel sono un labirinto pericoloso. Ti perdi e devi tornare indietro rapidamente.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Saccheggiare l'ingresso (Tracce)",
					"requirements": {"tracce": 10},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 10,
						"success": {
							"text": "Nelle biglietterie e nei negozi dell'atrio trovi oggetti dimenticati e materiali utili.",
							"rewards": {"items": {"scrap_metal": 2, "mechanical_parts": 1, "cloth_rags": 2}}
						},
						"failure": {
							"text": "L'atrio è stato già saccheggiato a fondo. Solo detriti inutili.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Sigillare l'ingresso",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Usi detriti per bloccare l'accesso ai tunnel. Qualunque cosa ci sia laggiù, è meglio che rimanga sepolta.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_unstable_skyscraper": {
			"id": "city_unstable_skyscraper",
			"name": "Grattacielo Pericolante",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Un grattacielo con enormi squarci nella facciata incombe minacciosamente. Sembra possa crollare da un momento all'altro, ma i piani alti potrebbero contenere risorse intatte.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Salire ai piani alti (Agilità)",
					"requirements": {"agilita": 15},
					"consequences": {
						"action": "skill_check",
						"stat": "agilita",
						"difficulty": 15,
						"success": {
							"text": "Con agilità e fortuna, raggiungi i piani superiori. Trovi uffici intatti con provviste e equipaggiamento.",
							"rewards": {"items": {"ration_pack": 2, "water_bottle": 1, "weapon_knife": 1, "mechanical_parts": 2}}
						},
						"failure": {
							"text": "Una sezione del pavimento cede sotto i tuoi piedi. Scappi a malapena dal crollo parziale.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Esplorare il piano terra (Tracce)",
					"requirements": {"tracce": 11},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 11,
						"success": {
							"text": "Nell'atrio e nei negozi del piano terra trovi materiali e oggetti abbandonati.",
							"rewards": {"items": {"scrap_metal": 3, "cloth_rags": 2, "charcoal": 1}}
						},
						"failure": {
							"text": "Il piano terra è instabile e pericoloso. Meglio non rischiare.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Girare al largo",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Decidi che il grattacielo è troppo pericoloso. Ti allontani rapidamente dalla zona di possibile crollo.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_ghost_market": {
			"id": "city_ghost_market",
			"name": "Il Mercato Nero Fantasma",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Ti imbatti in una serie di bancarelle improvvisate e ormai deserte, nascoste in un vicolo. Sembra un vecchio mercato nero. Qualcosa di valore potrebbe essere stato dimenticato.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Frugare nelle bancarelle (Tracce)",
					"requirements": {"tracce": 12},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 12,
						"success": {
							"text": "Tra le bancarelle abbandonate trovi merce nascosta e oggetti di valore.",
							"rewards": {"items": {"weapon_knife": 1, "ammo_generic": 1, "medicine_crude": 2, "scrap_metal": 1}}
						},
						"failure": {
							"text": "Le bancarelle sono state svuotate completamente. Solo polvere e ragnatele.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Investigare i gestori (Presagio)",
					"requirements": {"presagio": 13},
					"consequences": {
						"action": "skill_check",
						"stat": "presagio",
						"difficulty": 13,
						"success": {
							"text": "Trovi indizi sui mercanti e la posizione di una cache segreta.",
							"rewards": {"items": {"lore_fragment": 1, "map_fragment": 1, "mechanical_parts": 1}}
						},
						"failure": {
							"text": "Non riesci a capire chi gestiva il mercato o dove potrebbero aver nascosto la merce.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Usare come nascondiglio",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Il vicolo nascosto potrebbe essere un buon rifugio temporaneo. Ti riposi tra le bancarelle vuote.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_military_vehicle": {
			"id": "city_military_vehicle",
			"name": "Veicolo Militare Abbandonato",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Un veicolo militare corazzato, forse un carro armato o un trasporto truppe, è bloccato in mezzo alla strada, crivellato di colpi.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Entrare nel veicolo (Potenza)",
					"requirements": {"potenza": 13},
					"consequences": {
						"action": "skill_check",
						"stat": "potenza",
						"difficulty": 13,
						"success": {
							"text": "Forzi l'ingresso del veicolo blindato. All'interno trovi armi militari e equipaggiamento tattico.",
							"rewards": {"items": {"weapon_rifle": 1, "ammo_rifle": 2, "body_armor": 1}}
						},
						"failure": {
							"text": "Il veicolo è sigillato troppo bene. Non riesci a entrare senza attrezzi specializzati.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Usare come copertura",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Il veicolo blindato offre un'eccellente copertura. Ti riposi al sicuro dietro la sua corazza.",
						"rewards": {}
					}
				},
				{
					"text": "Recuperare parti metalliche",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Stacchi pezzi di blindatura e componenti metallici dal veicolo danneggiato.",
						"rewards": {"items": {"scrap_metal": 4}}
					}
				}
			]
		},
		
		"city_silent_hospital": {
			"id": "city_silent_hospital",
			"name": "L'Ospedale Silenzioso",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Le porte di un grande ospedale cittadino sono divelte. Dentro, il silenzio è rotto solo dal gocciolio dell'acqua e dal vento che sibila tra i corridoi bui.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Cercare nella farmacia (Adattamento)",
					"requirements": {"adattamento": 13},
					"consequences": {
						"action": "skill_check",
						"stat": "adattamento",
						"difficulty": 13,
						"success": {
							"text": "Nella farmacia dell'ospedale trovi medicine preziose e forniture mediche.",
							"rewards": {"items": {"antidote": 2, "vitamins": 2, "first_aid_kit": 1, "medicine_crude": 3}}
						},
						"failure": {
							"text": "La farmacia è stata saccheggiata o le medicine sono scadute e inutilizzabili.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Esplorare le cucine (Tracce)",
					"requirements": {"tracce": 11},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 11,
						"success": {
							"text": "Nelle cucine e nei distributori trovi cibo in scatola e acqua ancora potabile.",
							"rewards": {"items": {"canned_food": 2, "water_bottle": 2, "ration_pack": 1}}
						},
						"failure": {
							"text": "Le cucine sono vuote e i distributori sono stati svuotati.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Evitare l'ospedale",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Decidi che gli ospedali abbandonati sono troppo pericolosi. Malattie, gas tossici, o peggio potrebbero annidarsi dentro.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_propaganda_posters": {
			"id": "city_propaganda_posters",
			"name": "Manifesti di Propaganda Strappati",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Sui muri scrostati, resti di vecchi manifesti di propaganda promettono un futuro radioso che non è mai arrivato, o minacciano nemici ormai dimenticati.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Studiare i manifesti (Presagio)",
					"requirements": {"presagio": 11},
					"consequences": {
						"action": "skill_check",
						"stat": "presagio",
						"difficulty": 11,
						"success": {
							"text": "I manifesti rivelano informazioni sul passato e indizi su depositi governativi nascosti.",
							"rewards": {"items": {"lore_fragment": 2, "map_fragment": 1}}
						},
						"failure": {
							"text": "I manifesti sono troppo danneggiati per essere compresi. Solo propaganda senza senso.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Strapparli via",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Strappi via i manifesti in un gesto di sfida contro il passato. Un piccolo atto di ribellione.",
						"rewards": {}
					}
				},
				{
					"text": "Usare come combustibile",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "La carta dei manifesti può essere utile per accendere fuochi.",
						"rewards": {"items": {"charcoal": 2}}
					}
				}
			]
		},
		
		"city_overgrown_park": {
			"id": "city_overgrown_park",
			"name": "Il Parco Cittadino Invaso dalla Natura",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Un tempo un parco curato, ora è una giungla selvaggia dove la natura si è ripresa i suoi spazi tra statue incrinate e panchine arrugginite.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Cercare piante medicinali (Adattamento)",
					"requirements": {"adattamento": 12},
					"consequences": {
						"action": "skill_check",
						"stat": "adattamento",
						"difficulty": 12,
						"success": {
							"text": "Tra la vegetazione selvaggia trovi piante con proprietà curative e bacche commestibili.",
							"rewards": {"items": {"medicine_crude": 2, "berries": 3, "vitamins": 1}}
						},
						"failure": {
							"text": "Non riesci a distinguere le piante utili da quelle velenose. Meglio non rischiare.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Cacciare piccola selvaggina (Tracce)",
					"requirements": {"tracce": 13},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 13,
						"success": {
							"text": "Riesci a catturare piccoli animali che si sono adattati all'ambiente urbano.",
							"rewards": {"items": {"meat_raw": 2, "cloth_rags": 1}}
						},
						"failure": {
							"text": "Gli animali sono troppo veloci o diffidenti. Non riesci a catturare nulla.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Trovare un rifugio",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Tra la vegetazione fitta trovi un luogo appartato e sicuro per riposare.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_sewer_sounds": {
			"id": "city_sewer_sounds",
			"name": "Suoni dalla Fognatura",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Da un tombino aperto o una grata rotta provengono suoni inquietanti: raschiare, grugniti, o forse voci distorte.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Investigare i suoni (Presagio)",
					"requirements": {"presagio": 14},
					"consequences": {
						"action": "skill_check",
						"stat": "presagio",
						"difficulty": 14,
						"success": {
							"text": "Il tuo istinto ti guida verso una scoperta inquietante ma redditizia nelle fogne.",
							"rewards": {"items": {"scrap_metal": 1, "mechanical_parts": 2, "weapon_pipe": 1}}
						},
						"failure": {
							"text": "I suoni diventano più minacciosi. Ti allontani rapidamente, sentendoti osservato dal basso.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Sigillare l'apertura",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Usi detriti pesanti per sigillare il tombino. Qualunque cosa ci sia sotto, è meglio che rimanga lì.",
						"rewards": {}
					}
				},
				{
					"text": "Allontanarsi rapidamente",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "Decidi che alcuni misteri è meglio lasciarli irrisolti. Ti allontani velocemente dalla zona.",
						"rewards": {}
					}
				}
			]
		},
		
		"city_intact_apartment": {
			"id": "city_intact_apartment",
			"name": "L'Appartamento Intatto",
			"type": 0,  # LOCATION_SPECIFIC
			"description": "Incredibilmente, trovi un appartamento in un palazzo diroccato che sembra quasi intatto, come se i suoi occupanti fossero appena usciti. Polvere ovunque, ma mobili e oggetti personali sono al loro posto.",
			"image": "",
			"conditions": {},
			"choices": [
				{
					"text": "Saccheggiare con cura (Tracce)",
					"requirements": {"tracce": 10},
					"consequences": {
						"action": "skill_check",
						"stat": "tracce",
						"difficulty": 10,
						"success": {
							"text": "Esplori l'appartamento con rispetto, trovando provviste, vestiti e oggetti utili.",
							"rewards": {"items": {"canned_food": 2, "water_bottle": 1, "cloth_rags": 2, "first_aid_kit": 1}}
						},
						"failure": {
							"text": "L'appartamento sembra intatto ma è stato già svuotato di tutto il necessario.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Cercare un diario (Presagio)",
					"requirements": {"presagio": 12},
					"consequences": {
						"action": "skill_check",
						"stat": "presagio",
						"difficulty": 12,
						"success": {
							"text": "Trovi un diario che rivela informazioni preziose sui giorni finali e nascondigli segreti.",
							"rewards": {"items": {"lore_fragment": 2, "map_fragment": 1}}
						},
						"failure": {
							"text": "Non trovi tracce scritte della vita degli ex-occupanti. Il loro destino rimane un mistero.",
							"rewards": {}
						}
					}
				},
				{
					"text": "Usare come rifugio",
					"requirements": {},
					"consequences": {
						"action": "simple_result",
						"text": "L'appartamento intatto offre un rifugio sicuro e confortevole per la notte. Un lusso raro in questi tempi.",
						"rewards": {}
					}
				}
			]
		}
	} 